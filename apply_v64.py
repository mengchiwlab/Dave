...
print(f"Found doScore at {start_idx}, ends before showError at {end_idx}")
...
new_content = content[:start_idx] + new_doscore + content[end_idx:]

with open('/Users/wangmengchi/Desktop/AH评分/ah_ipo_v60_web.html', 'w') as f:
    f.write(new_content)

print("✅ V6.4 doScore replaced successfully")

# 验证
with open('/Users/wangmengchi/Desktop/AH评分/ah_ipo_v60_web.html', 'r') as f:
    check = f.read()
    
assert 'V6.4 成熟系数模型' in check
assert 'function doScore()' in check
assert 'function showError(msg)' in check
