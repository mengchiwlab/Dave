...
def calc_score_v59(row):
    ...
    return s_ret + s_beta + s_cs + s_pos + s_alpha + s_disc + s_ind + s_cap + s_sp
...
def calc_score_v60(row):
    ...
    if direction >= 0:
        s_beta_raw = beta_base * direction
    else:
        s_beta_raw = (21 - beta_base) * abs(direction)
    # 行业加权
    s_beta = s_beta_raw * ind_beta_weight
    s_beta = min(21, s_beta)  # 封顶
    ...
    if r > -5 and disc > 40:
        interaction_score = 8
    elif r > -5 and disc > 30:
        interaction_score = 5
    elif r > -15 and disc > 40:
        interaction_score = 3
    elif disc > 30:
        interaction_score = 1
    s_interact = interaction_score

    # 9. 行业 (15分)
    s_ind = ind_base * 3.0  # 满分15
    ...
    total = s_ret + s_beta + s_cs + s_pos + s_vol + s_disc + s_interact + s_ind + s_cap + s_sp
...
def main():
    print("=" * 70)
    print("📊 AH股IPO评分系统 V6.0 - 五大改进整合回测")
    print("=" * 70)
    ...
    # V5.8 原始
    r1 = run_backtest(df, calc_score_v59, "V5.9 原始版")
    results.append(r1)
    print_results(r1)

    # V6.0 改进
    r2 = run_backtest(df, calc_score_v60, "V6.0 改进版")
    results.append(r2)
    print_results(r2)
    ...
    df["score_v59"] = df.apply(calc_score_v59, axis=1)
    df["score_v60"] = df.apply(calc_score_v60, axis=1)

    samples = df.nlargest(5, "score_v60")[["correct_name", "ret45", "beta_real", "return_1d",
                                              "score_v59", "score_v60"]]
    print(f"\n  V6.0 Top5 股票:")
    print(f"  {'名称':<10} {'ret45':>8} {'Beta':>6} {'首日':>8} {'V5.9':>6} {'V6.0':>6}")
    for _, row in samples.iterrows():
        print(f"  {row['correct_name']:<8} {row['ret45']:>+7.1f}% {row['beta_real']:>5.2f} {row['return_1d']:>+7.1f}% {row['score_v59']:>6.0f} {row['score_v60']:>6.0f}")

    # 保存结果
    df.to_csv("output/ah_ipo_backtest_v60.csv", index=False, encoding="utf-8-sig")
    print(f"\n💾 回测数据已保存: output/ah_ipo_backtest_v60.csv")
    print("=" * 70)
